...
module.exports = {
    name: 'antilink',
    description: 'Protection anti-liens',
    ...
    async execute(client, message, args) {
        ...
        const mod = getModule(message.guild.id, 'antilink');
        ...
        if (!sub || sub === 'config') {
            ...
            container.addTextDisplayComponents(
                new TextDisplayBuilder().setContent(
                    `-# "+antilink on" · "+antilink off" · "+antilink action <action>"`
                )
            );
        ...
        if (sub === 'on' || sub === 'off') {
            mod.enabled = sub === 'on';
            saveModule(message.guild.id, 'antilink', mod);
            return message.reply(
                `✅ **Protection anti-liens** ${mod.enabled ? 'activé' : 'désactivé'}.`
            );
        }
        ...
        return message.reply(
            `❌ Sous-commande inconnue. Utilisez "+antilink config".`
        );
    }
};
