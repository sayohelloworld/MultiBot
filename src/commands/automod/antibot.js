...
module.exports = {
    name: 'antibot',
    description: 'Protection anti-bot',
    ...
    async execute(client, message, args) {
        ...
        const mod = getModule(message.guild.id, 'antibot');
        ...
        if (!sub || sub === 'config') {
            ...
            container.addTextDisplayComponents(
                new TextDisplayBuilder().setContent(
                    `-# "+antibot on" · "+antibot off" · "+antibot action <action>"`
                )
            );
        ...
        if (sub === 'on' || sub === 'off') {
            mod.enabled = sub === 'on';
            saveModule(message.guild.id, 'antibot', mod);
            return message.reply(
                `✅ **Protection anti-bot** ${mod.enabled ? 'activé' : 'désactivé'}.`
            );
        }
        ...
        return message.reply(
            `❌ Sous-commande inconnue. Utilisez "+antibot config".`
        );
    }
};
