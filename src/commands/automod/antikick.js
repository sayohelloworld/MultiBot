...
module.exports = {
    name: 'antikick',
    description: 'Protection anti-kick',
    ...
    async execute(client, message, args) {
        ...
        const mod = getModule(message.guild.id, 'antikick');
        ...
        if (!sub || sub === 'config') {
            ...
            container.addTextDisplayComponents(
                new TextDisplayBuilder().setContent(
                    `-# "+antikick on" · "+antikick off" · "+antikick action <action>"`
                )
            );
        ...
        if (sub === 'on' || sub === 'off') {
            mod.enabled = sub === 'on';
            saveModule(message.guild.id, 'antikick', mod);
            return message.reply(
                `✅ **Protection anti-kick** ${mod.enabled ? 'activé' : 'désactivé'}.`
            );
        }
        ...
        return message.reply(
            `❌ Sous-commande inconnue. Utilisez "+antikick config".`
        );
    }
};
