...
module.exports = {
    name: 'antijoin',
    description: 'Protection anti-raid',
    ...
    async execute(client, message, args) {
        ...
        const mod = getModule(message.guild.id, 'antijoin');
        ...
        if (!sub || sub === 'config') {
            ...
            container.addTextDisplayComponents(
                new TextDisplayBuilder().setContent(
                    `-# "+antijoin on" · "+antijoin off" · "+antijoin action <action>"`
                )
            );
        ...
        if (sub === 'on' || sub === 'off') {
            mod.enabled = sub === 'on';
            saveModule(message.guild.id, 'antijoin', mod);
            return message.reply(
                `✅ **Protection anti-raid** ${mod.enabled ? 'activé' : 'désactivé'}.`
            );
        }
        ...
        return message.reply(
            `❌ Sous-commande inconnue. Utilisez "+antijoin config".`
        );
    }
};
