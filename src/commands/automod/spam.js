...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        const mod = getModule(message.guild.id, 'spam');
        ...
        if (!sub || sub === 'config') {
            ...
            container.addTextDisplayComponents(
                new TextDisplayBuilder().setContent(
                    `-# "+spam on" · "+spam off" · "+spam action <action>"`
                )
            );
        ...
        if (sub === 'on' || sub === 'off') {
            mod.enabled = sub === 'on';
            saveModule(message.guild.id, 'spam', mod);
            return message.reply(
                `✅ **Protection anti-spam** ${mod.enabled ? 'activé' : 'désactivé'}.`
            );
        }
        ...
        return message.reply(
            `❌ Sous-commande inconnue. Utilisez "+spam config".`
        );
    }
};
