...
module.exports = {
    name: 'role',
    description: 'Protection des rôles',
    ...
    async execute(client, message, args) {
        ...
        const mod = getModule(message.guild.id, 'role');
        ...
        if (!sub || sub === 'config') {
            ...
            container.addTextDisplayComponents(
                new TextDisplayBuilder().setContent(
                    `-# "+role on" · "+role off" · "+role action <action>"`
                )
            );
        ...
        if (sub === 'on' || sub === 'off') {
            mod.enabled = sub === 'on';
            saveModule(message.guild.id, 'role', mod);
            return message.reply(
                `✅ **Protection des rôles** ${mod.enabled ? 'activé' : 'désactivé'}.`
            );
        }
        ...
        return message.reply(
            `❌ Sous-commande inconnue. Utilisez "+role config".`
        );
    }
};
